...
def main():
    ...
    ap.add_argument("--cc-pitches", type=float, nargs="+",
                    ...
                    help="Elevation angles to sweep (degrees). "
                         "Add more values to cover tilted checkers.")
    ...
    if checker_src and HAVE_CC_ERP:
        ...
        if cc_measured is not None:
            # WB from patch #19 — the white patch (N9.5), index 18.
            # This is the brightest neutral on the chart, highest SNR.
            # Simple: scale_c = reference_c / measured_c
            # then luminance-normalise so the overall brightness is unchanged.
            white_patch_idx = 18   # CC24 patch 19, 0-based index
            meas  = cc_measured[white_patch_idx]                  # (3,) linear RGB
            ref   = CC24_LINEAR_SRGB[white_patch_idx]             # (3,) linear RGB
            log(f"CC white patch (#19) measured linear RGB: "
                f"R={meas[0]:.5f} G={meas[1]:.5f} B={meas[2]:.5f}")
            log(f"CC white patch (#19) reference linear RGB: "
                f"R={ref[0]:.5f} G={ref[1]:.5f} B={ref[2]:.5f}")
            scale_raw = ref / np.clip(meas, 1e-8, None)           # (3,)
            # Luminance-neutral normalisation
            luma_s = float(0.2126*scale_raw[0]
                           + 0.7152*scale_raw[1]
                           + 0.0722*scale_raw[2])
            wb_from_chart_scale = (scale_raw / max(luma_s, 1e-8)).astype(np.float32)
            wb_from_chart = True
            log(f"CC WB scale (luma-neutral): "
    ...
    # ── 4. White balance ──────────────────────────────────────────────────
    # Priority: chart neutrals > explicit rgb_scale/kelvin/swatch > sphere WB > dome WB
    wb_img, wb_scale, wb_dome_info = apply_white_balance(
        img, kelvin=args.kelvin, rgb_scale=rgb_scale,
        swatch_xy=wb_swatch_xy, dome_wb_mode=args.dome_wb,
        sphere_wb=args.sphere_wb and not wb_from_chart)
    save_png_preview(os.path.join(args.debug_dir, "01_wb_preview.png"), wb_img)
    meta["white_balance"] = {
        "source":        "colorchecker_neutrals" if wb_from_chart else (
                         "sphere_render" if args.sphere_wb else
                         args.dome_wb or "manual/none"),
        "wb_scale":      wb_scale.tolist(),
        "kelvin":        args.kelvin,
        "rgb_scale_arg": rgb_scale.tolist() if hasattr(rgb_scale, 'tolist') else rgb_scale,
        "dome_wb_mode":  args.dome_wb,
        "sphere_wb":     args.sphere_wb and not wb_from_chart,
    ...
    # WB was already applied from the white patch in step 4.
    # No residual matrix — keep it simple.
    if cc_measured is not None:
        cc_info = {
            "applied":          True,
            "source":           checker_src,
            "wb_from_patch19":  True,
            "detection":        cc_det_info,
            "wb_scale":         wb_from_chart_scale.tolist() if wb_from_chart else None,
            "note":             "WB derived from CC24 patch #19 (white N9.5). No 3×3 matrix.",
        }
    else:
        cc_info = {"applied": False,
